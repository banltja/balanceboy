 ...
 */
void balanceModel_IfActionSubsystem(real_T *rty_Out1,
...
void balanceModel_step(void)
{
  ...
  if (rtmIsMajorTimeStep(balanceModel_M)) {
     ...
     *  S-Function (ev3_buttons): '<Root>/Button'
     */
    tmp = (int8_T)proximity;
    rtb_Encoder = getEncoderValueExternalReset(1U, &tmp);

    /* If: '<S6>/If' incorporates:
     *  S-Function (ev3_encoder): '<Root>/Encoder'
     */
    if (rtsiIsModeUpdateTimeStep(&balanceModel_M->solverInfo)) {
      balanceModel_DW.If_ActiveSubsystem = (int8_T)((rtb_Encoder >= 70) ||
        (rtb_Encoder <= -70));
    }
    ...
    switch (balanceModel_DW.If_ActiveSubsystem) {
     case 0:
       ...
       */
      balanceModel_IfActionSubsystem(&balanceModel_B.Merge,
     ...
     *  MATLABSystem: '<Root>/Infrared Sensor'
     */
    rtb_Sum_j = balanceModel_P.Constant_Value - (real_T)rtb_InfraredSensor_0;

    /* Gain: '<S43>/Proportional Gain' */
    balanceModel_B.ProportionalGain = balanceModel_P.PIDController1_P *
      rtb_Sum_j;

    /* Gain: '<S32>/Derivative Gain' */
    balanceModel_B.DerivativeGain = balanceModel_P.PIDController1_D * rtb_Sum_j;
  }
  ...
  balanceModel_B.FilterCoefficient = (balanceModel_B.DerivativeGain -
    balanceModel_X.Filter_CSTATE) * balanceModel_P.PIDController1_N;
  if (rtmIsMajorTimeStep(balanceModel_M)) {
     ...
     *  Sum: '<S47>/Sum'
     */
    tmp_1 = floor(((balanceModel_B.ProportionalGain +
                    balanceModel_X.Integrator_CSTATE) +
                   balanceModel_B.FilterCoefficient) * balanceModel_B.Merge);
     ...
     */
    lcdDisplay(tmp_0 < 0.0 ? -(int32_T)(uint32_T)-tmp_0 : (int32_T)(uint32_T)
               tmp_0, balanceModel_ConstP.LCD_p1_o, 2U, 1U);

    /* S-Function (ev3_lcd): '<S3>/LCD' */
    lcdDisplay(rtb_Encoder, balanceModel_ConstP.LCD_p1_b, 3U, 1U);
    ...
    balanceModel_B.IntegralGain = balanceModel_P.PIDController1_I * rtb_Sum_j;
  }
...
void balanceModel_initialize(void)
{
  ...
  balanceModel_X.Integrator_CSTATE =
    balanceModel_P.PIDController1_InitialConditi_k;

  /* InitializeConditions for Integrator: '<S33>/Filter' */
  balanceModel_X.Filter_CSTATE = balanceModel_P.PIDController1_InitialCondition;
